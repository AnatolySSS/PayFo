...
export function osagoPenaltyParagraph(paymentVoluntary,
    ...
    <p>В пункте 76 Постановления Пленума ВС РФ № 31 указано, что неустойка 
    исчисляется со дня, следующего за днем, установленным для принятия решения о выплате страхового 
    возмещения, то есть с 21-го дня после получения страховщиком заявления потерпевшего о страховой 
    выплате и документов, предусмотренных Правилами, и до дня фактического исполнения страховщиком 
    обязательства по договору включительно. </p>`
    ...
    if (paymentVoluntary[i].type.options.selectedIndex != 0) {
        ...
        case 2:
            ...
            claim_add_motivation[i] = `<p>Согласно пункту 19 Постановление Пленума № 31 
            при наступлении страхового случая потерпевший обязан не только уведомить 
            ...
            предусмотренные Правилами ОСАГО. Кроме сведений о расходах на восстановление поврежденного 
            имущества потерпевший должен также сообщить о другом известном ему 
            ущербе, который подлежит возмещению (например, об утрате товарной 
            стоимости, о расходах на эвакуацию транспортного средства с места 
            дорожно-транспортного происшествия и т.п.).</p>
            <p>Согласно пункту 40 Постановление Пленума № 31 к реальному ущербу, возникшему 
            в результате дорожно-транспортного происшествия, относится также утрата товарной стоимости, 
            которая представляет собой уменьшение стоимости транспортного средства.</p>`
            break;
        case 3:
        ...
        if (paymentVoluntary[i].days_delay <= 0) {
            ...
            размер неустойки, подлежащей выплате за период с ${paymentVoluntary[i].penalty_day_form} по ${paymentVoluntary[i].getDateFormatted()}, 
    ...
    if (!isNaN(paymentFu[i].getDate())) {
        ...
        //Если данное требование было удовлетворено ФУ
                ...
                if ((paymentFu[i].claim[j].name.options.selectedIndex == 1 ||
                    ...
                    paymentFu[i].claim[j].type.options.selectedIndex == 1) {
                        ...
                        case 2:
                            ...
                            claim_fu_add_motivation[i][j] = `<p>Согласно пункту 19 Постановление Пленума № 31 
                            при наступлении страхового случая потерпевший обязан не только уведомить 
                            страховщика о его наступлении в сроки, установленные Правилами ОСАГО, 
                            но и направить страховщику заявление о страховом возмещении и документы, 
                            предусмотренные Правилами ОСАГО. Кроме сведений о расходах на восстановление поврежденного 
                        ...
                        <p>В соответствии с требованиями, установленными пунктом 21 статьи 12 Закона № 40-ФЗ, 
                        размер неустойки, подлежащей выплате за период с ${paymentFu[i].claim[j].penalty_day_form} по 
            ...
            if ((paymentCourt[i].claim[j].name.options.selectedIndex == 1 ||
                ...
                paymentCourt[i].claim[j].type.options.selectedIndex == 1) {
                    ...
                    case 2:
                        ...
                        claim_court_add_motivation[i][j] = `<p>Согласно пункту 19 Постановление Пленума № 31 
                        при наступлении страхового случая потерпевший обязан не только уведомить 
                        страховщика о его наступлении в сроки, установленные Правилами ОСАГО, 
                        но и направить страховщику заявление о страховом возмещении и документы, 
                        предусмотренные Правилами ОСАГО. Кроме сведений о расходах на восстановление поврежденного 
                    ...
                    <p>В соответствии с требованиями, установленными пунктом 21 статьи 12 Закона № 40-ФЗ, 
                    размер неустойки, подлежащей выплате за период с ${paymentCourt[i].claim[j].penalty_day_form} по 
    ...
    //Добавление условной неустойки
    let conditional_penalty_paragraph = ""
    let conditional_penalty_boolean = false
    if (total_summ > 0) {
        conditional_penalty_boolean = true
        ...
        неустойки взысканной Финансовым уполномоченным в размере ${makeRubText_genitive(total_penalty_summ_accrued)}`
        }
        ...
        conditional_penalty_paragraph = `<p>Согласно экспертному заключению ${organization} от ${date_nte} № ${number}, 
        проведенному по инициативе Финансового уполномчоенного, с Финансовой организации подлежит взысканию страховое 
        возмещение в размере ${makeRubText_genitive(total_summ)}.</p>
        <p>В соответствии с пунктом 65 Постановления Пленума ВС РФ № 7 по смыслу статьи 330 ГК РФ, истец вправе требовать присуждения неустойки по день фактического 
        исполнения обязательства (в частности, фактической уплаты кредитору денежных средств, передачи товара, 
        завершения работ).</p>
        ${max_summ_paragraf}
        <p>Соответственно, удовлетворению подлежит требование Заявителя о взыскании неустойки начиная 
        с ${paymentVoluntary[0].date_sv.getPenaltyDayFormatted()} по дату фактического исполнения Финансовой организацией своего 
        обязательства перед Заявителем по выплате страхового возмещения в размере ${makeRubText_genitive(total_summ)}, 
        исходя из ставки 1% за каждый день просрочки, но не более ${makeRubText_genitive(max_summ)}${jointly_helper}.</p>
        <p>Вместе с тем, в силу статьи 24 Закона № 123-ФЗ исполнение финансовой организацией вступившего в силу решения 
        финансового уполномоченного признается надлежащим исполнением финансовой организацией обязанностей по 
        соответствующему договору с потребителем финансовых услуг об оказании ему или в его пользу финансовой услуги.</p>
        <p>Согласно части 5 статьи 16.1 Закона № 40-ФЗ страховщик освобождается от обязанности уплаты неустойки (пени), 
        суммы финансовой санкции и (или) штрафа, если обязательства страховщика были исполнены в порядке и в сроки, 
        которые установлены Законом № 40-ФЗ, Законом № 123-ФЗ, а также если страховщик докажет, что нарушение сроков 
        произошло вследствие непреодолимой силы или по вине потерпевшего.</p>
        <p>Таким образом, неустойка, начисляемая на сумму ${makeRubText_genitive(total_summ)}, подлежит взысканию только в случае 
        неисполнения (неисполнения в установленные сроки) Финансовой организацией решения Финансового уполномоченного 
        по настоящему спору в части удовлетворения основного требования (о взыскании страхового возмещения).</p>`
    }
    ...
    out_data = {
        ...
        conditional_penalty_boolean : conditional_penalty_boolean,
    }
